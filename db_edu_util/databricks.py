...
def _map_rest_error(json_data: Dict[str, str]) -> Tuple[StatusCode, Optional[str]]:
    """
...
class RESTClient(object):
    ...
    def __init__(self, profile: str = "DEFAULT", config: str = "~/.databrickscfg"):
        """
        ...
        if profile is None:
            profile = "DEFAULT"
        (self._host, self._token, self._home) = self._get_profile(profile, config)
    ...
    def _get_profile(self, profile: str, config: str) -> Tuple[str, str, Optional[str]]:
        config_file = os.path.expanduser(config)
        if not os.path.exists(config_file):
            raise DatabricksError(
                message=f'"{config_file}" does not exist.', code=StatusCode.CONFIG_ERROR
            )
    ...
    def _issue_post(
        self, url: str, params: Dict[str, Any], file: Optional[str] = None
    ) -> requests.Response:
        ...
        if file:
            resp = requests.post(
                url, data=params, headers=headers, files={"file": open(file, "rb")}
            )
        else:
...
class Workspace(RESTClient):
        ...
        try:
            ...
            for f in data.get("objects", []):
                ...
                if t == ObjectType.NOTEBOOK:
                    ...
                    lang = None
                files.append(ObjectInfo(path=f["path"], object_type=t, language=lang))
            return files
    ...
    def import_notebook(
        self, local_path: str, workspace_path: str, overwrite: bool = False
    ) -> NoReturn:
        """
    ...
    def import_dir(
        self, local_dir: str, workspace_path: str, overwrite: bool = False
    ) -> NoReturn:
        """
    ...
    def export_dir(self, workspace_path: str, local_dir: str) -> NoReturn:
        ...
        if os.path.isfile(local_dir):
            raise DatabricksError(
                code=StatusCode.ALREADY_EXISTS,
                message=f"Local directory {local_dir} exists and is not a " "directory",
            )
    ...
    def export_dbc(self, workspace_path: str, local_path: str):
        ...
        url = self._workspace_url("export")
        try:
            params = {"path": self._adjust_remote_path(workspace_path), "format": "DBC"}
            resp = self._issue_get(url, params)
            data = resp.json()
            if "content" not in data:
                raise DatabricksError('No "content" in JSON response.')
