    ...
    def move_notebooks(master):
        ...
        for lang in set(EXT_LANG.values()):
            ...
            if master[lc_lang] and master[p]:
                ...
                no_more_student_parts = False
                student_part_num = 1
                while(not no_more_student_parts):
                    ...
                    if path.exists(student_part_nb):
                        verbose("moving student nb part " + student_part_nb)
                        move(student_part_nb,
                             path.join(dest_root,
                                       LABS_SUBDIR,
                                       lang,
                                       '{0}_part{1}_student{2}'.format(
                        ...
                        verbose("student part nb path does not exist: " + \
                                student_part_nb)
                        no_more_student_parts = True
                    student_part_num += 1

                # Move answer part notebooks, if any.
                no_more_answer_parts = False
                answer_part_num = 1
                while(not no_more_answer_parts):
                    ...
                    if path.exists(answer_part_nb):
                        ...
                        verbose("answer part nb path does not exist: " + \
                                answer_part_nb)
                        no_more_answer_parts = True
                    answer_part_num += 1
                ...
                if not path.exists(student_nb):
                    verbose("student nb path does not exist: " + student_nb)
                    raise BuildError("Can't find expected notebook {0}".format(
                        student_nb
                    ))

                # Compute a suffix for the student nb w/ all the parts.
                stud_suf_text = "_parts_{0}-{1}".format(1, student_part_num-2)
                stud_parts_suf = stud_suf_text if student_part_num > 2 else ""
                ...
                answer_suf_text = "_parts_{0}-{1}".format(1, answer_part_num-2)
                answ_parts_suf = answer_suf_text if answer_part_num > 2 else ""
