...
def _icon_image_url(image):
    return "https://files.training.databricks.com/static/images/" + image
...
VALID_CELL_TYPES_FOR_LABELS = {
    CommandLabel.IPYTHON_ONLY: CODE_CELL_TYPES
    | {CommandCode.MARKDOWN, CommandCode.MARKDOWN_SANDBOX},
    ...
    CommandLabel.INSTRUCTOR_NOTE: {CommandCode.MARKDOWN, CommandCode.MARKDOWN_SANDBOX},
    CommandLabel.ALL_NOTEBOOKS: ALL_CELL_TYPES,
    CommandLabel.VIDEO: {CommandCode.MARKDOWN, CommandCode.MARKDOWN_SANDBOX},
...
<a target="_blank" href="https://fast.wistia.net/embed/iframe/${id}?seo=false">
  <img alt="Opens in new tab" src="'''
    + _icon_image_url("external-link-icon-16x16.png")
    + """"/>&nbsp;Watch full-screen.</a>
</div>"""
)
...
class Params:
    ...
    def __init__(
        ...
        for purpose, file in (
            ("Notebook footer", notebook_footer_path),
...
class NotebookGenerator(object):
    ...
    param_to_label = {
        ...
        CommandCode.LOAD_EXT: {CommandLabel.PYTHON_ONLY},
        CommandCode.PIP: {CommandLabel.PYTHON_ONLY},
        CommandCode.TENSORBOARD: {CommandLabel.PYTHON_ONLY},
        ...
        NotebookUser.ANSWERS: {CommandLabel.ANSWER, CommandLabel.PRIVATE_TEST},
    ...
    def __init__(
        ...
        self.keep_labels = self._get_keep_labels(
            params, notebook_kind, notebook_user, notebook_code
        )
    ...
    def _get_extension(self) -> str:
        return (
            NotebookGenerator.user_to_extension[self.notebook_user]
            + Parser.code_to_extension[self.notebook_code]
        )

    def _check_cell_type(
        self, command_code: CommandCode, labels: Set[CommandLabel], cell_num: int
    ) -> NoReturn:
        for label in labels:
    ...
    def generate(
        ...
        """
        verbose(f'Generating {self.notebook_user} notebook(s) for "{input_name}"')
        ...
        if parts:
            ...
            max_part = max([dataclasses.asdict(c)["part"] for c in commands])
        else:
            max_part = 0
        ...
        for i in range(max_part + 1):  # parts are zero indexed
            ...
            file_out = os.path.join(out_dir, base_file + part_string + self.file_ext)
            if is_IPython:
                file_out = file_out.replace(".py", ".ipynb")

            magic_prefix = f"{self.base_comment} MAGIC"
            with codecs.open(
                file_out, "w", _file_encoding_out, errors=_file_encoding_errors
            ) as output:
                ...
                for cell_num, cmd in enumerate(commands):
                    ...
                    if CommandLabel.SOURCE_ONLY in labels:
                        ...
                        debug(f"Cell #{cell_num} is source-only. Suppressing it.")
                        continue
                    ...
                    if CommandLabel.INSTRUCTOR_NOTE in labels:
                        ...
                        content = [INSTRUCTOR_NOTE_HEADING] + content
                    ...
                    if code in (CommandCode.MARKDOWN, CommandCode.MARKDOWN_SANDBOX):
                        ...
                        (content, sandbox) = expand_inline_tokens(
                    ...
                    profiles = []
                    if (
                        (CommandLabel.PROFILES in labels)
                        or (CommandLabel.AMAZON_ONLY in labels)
                        or (CommandLabel.AZURE_ONLY in labels)
                    ):
                        profiles = self._get_profiles(input_name, cell_num, content)

                    if (
                        (params.active_profile is not None)
                        and (len(profiles) > 0)
                        and (params.active_profile.name not in profiles)
                    ):
                        # This cell is not in the right profile.
                        debug(
                            f'"{input_name}", cell #{cell_num}: Build '
                    ...
                    if (not (discard_labels & labels)) and (
                        (
                            (inline and code != self.notebook_code)
                            or ((not inline) and code == self.notebook_code)
                        )
                        or all_notebooks
                    ):

                        content = self.remove_and_replace(
                            content, code, inline, all_notebooks, is_first
                        )
                        ...
                        is_first = self._write_command(
                            output, cell_split, content, is_first
                        )
        ...
        if (len(consolidated_instructor_notes) > 0) and (params.instructor_notes_file):
            self._write_consolidated_instructor_notes(
                consolidated_instructor_notes, params.instructor_notes_file
            )

    def _write_consolidated_instructor_notes(
        self, content: Sequence[str], out: str
    ) -> NoReturn:
    ...
    def _handle_todo_cell(self, cell_num: int, content: List[str]) -> List[str]:
        ...
        def starts_with_comment(line):
            if comment_start.search(line) or (len(line.strip()) == 0):
                return True
            else:
                return False
    ...
    def _handle_test_cell(self, cell_num: int, content: List[str]) -> List[str]:
        new_content = []
        for line in content:
            m = _test.match(line)
            if not m:
    ...
    def _get_profiles(
        self, notebook_name: str, cell_num: int, content: List[str]
    ) -> Set[str]:
        result = set()
        for line in content:
    ...
    def _handle_video_cell(self, cell_num: int, content: List[str]) -> List[str]:
        new_content = []
        for line in content:
            m = _video.match(line)
            if not m:
    ...
    def _write_command(
        self, output: TextIO, cell_split: str, content: List[str], is_first: bool
    ) -> bool:
    ...
    def generate_ipynb(self, file_out: str) -> NoReturn:
        ...
        with codecs.open(
            file_out, "r", _file_encoding_out, errors=_file_encoding_errors
        ) as intermediate:
...
class CellTemplateProcessor(object):
    ...
    HINTS_PRELUDE_TEMPLATE = """<script type="text/javascript">
...
"""
    ...
    HINT_TEMPLATE = """<div class="hint-{{id_prefix}}" style="padding-bottom: 20px; display: none">
  Hint:
...
"""
    ...
    ANSWER_TEMPLATE = """<div id="answer-{{id_prefix}}" style="padding-bottom: 20px; display: none">
  The answer:
...
"""
    ...
    def process(
        ...
        def handle_hints(text: str) -> str:
            ...
            prelude_vars = {"id_prefix": self._id_prefix}
            prelude = self._renderer.render(self.HINTS_PRELUDE_TEMPLATE, prelude_vars)
        ...
        def handle_answer(text: str) -> str:
            ...
            vars = {"id_prefix": self._id_prefix}
...
def _regex_list_match(line: str, regex_list: Sequence[Pattern]) -> Optional[Match]:
    """
...
def _transform_match(lines: List[str], regex_list: Sequence[Pattern]) -> List[str]:
    """
...
def _replace_line(line: str, regex_replace_list: Sequence[Tuple[Pattern, str]]) -> str:
...
def _replace_text(
    lines: List[str], regex_replace_list: Sequence[Tuple[Pattern, str]]
) -> List[str]:
    """
...
class Parser:
    ...
    pattern_to_label = [
        (_private_test, CommandLabel.PRIVATE_TEST),
    ...
    pattern_to_code = [
        (_markdown, CommandCode.MARKDOWN),
    ...
    def generate_commands(
        self, file_name: str, params: Params
    ) -> Tuple[str, Sequence[Command]]:
        ...
        def extend_content(cell_state):
            debug(
                f'Notebook "{file_name}": Cell at line '
        ...
        assert (
            file_extension in Parser.extension_to_code
        ...
        with codecs.open(
            file_name, "r", _file_encoding_in, errors=_file_encoding_errors
        ) as dbcExport:
...
def main():
    ...
    version_or_file.add_argument(
        "filename", nargs="?", help="enter either a filename or a directory"
    )
    version_or_file.add_argument(
    ...
    targetGroup.add_argument(
        "-ip", "--ipython", help="generate ipython notebook(s)", action="store_true"
    )

    codeGroup = arg_parser.add_argument_group("code")
    codeGroup.add_argument(
        "-py", "--python", help="generate python notebook(s)", action="store_true"
    )
    codeGroup.add_argument(
        "-r", "--rproject", help="generate r notebook(s)", action="store_true"
    )
    codeGroup.add_argument(
        "-sc", "--scala", help="generate scala notebook(s)", action="store_true"
    )
    codeGroup.add_argument(
        "-sq", "--sql", help="generate sql notebook(s)", action="store_true"
    )

    userGroup = arg_parser.add_argument_group("user")
    userGroup.add_argument(
        "-an", "--answers", help="generate answers notebook(s)", action="store_true"
    )
    userGroup.add_argument(
        "-ex", "--exercises", help="generate exercises notebook(s)", action="store_true"
    )
    userGroup.add_argument(
        "-in",
        "--instructor",
    ...
    arg_parser.add_argument(
        "-ct",
        "--course-type",
        help='Course type, either "ilt" or "self-paced". ' + 'Default: "self-paced"',
        metavar="<coursetype>",
    ...
    arg_parser.add_argument(
        "-d",
        "--dir",
        help=("Base output directory. Default: " + f"{DEFAULT_OUTPUT_DIR}"),
        action="store",
        dest="output_dir",
        metavar="OUTPUT_DIR",
    )
    ...
    arg_parser.add_argument(
        "--profile",
        help="Build profile, if any. Be sure to specify " + "--all-profiles, as well.",
        metavar="NAME[=VALUE]",
        default=None,
    )
    arg_parser.add_argument(
        "--all-profiles",
        help="Comma-separated list of all build profiles " + "supported by the course.",
        metavar="PROFILES",
        default=None,
    )
    arg_parser.add_argument(
    ...
    if profile:
        if not all_profiles:
            warn(
                f"You've specified --profile, but not --all-profiles. "
                + "This is usually an error."
            )
        else:
            all_profiles = [Profile(p, p) for p in re.split(r"[\s,]+", all_profiles)]
